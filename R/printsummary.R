...
lapply(final_list, function(x) write.table( data.frame(x), 'test_bench.csv'  , append= T, sep=',' ))
...
data_prep_user_input <- function(df_full, dv) {
data<-df_full
...
##read csv file, and 3 lists
preprocessing_user_input2 <- function(conv_var_names){
...
return(final_data_after_processing)
   ...
   df_cleaned<-preprocessing_user_input2(conv_var_names);
   #df_cleaned<-df_full
